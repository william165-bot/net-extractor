...
export default async (req: Request) => {
  ...
  if (path.endsWith('/pay/verify')) {
    if (req.method !== 'GET' && req.method !== 'POST') return new Response(JSON.stringify({ error: 'Method not allowed' }), { status: 405 })
    try {
      const txId = (qp(req.url, 'transaction_id') || qp(req.url, 'transactionId') || qp(req.url, 'tx_id') || '').trim()
      if (!txId) return new Response(JSON.stringify({ error: 'Transaction ID required' }), { status: 400 })
      if (!/^\d+$/.test(txId)) return new Response(JSON.stringify({ error: 'Invalid Transaction ID' }), { status: 400 })
      ...
      let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ error: 'Session expired' }), { status: 401 }) }
      const userEmail = session.email.toLowerCase()
      const flw = await verifyFlw(txId); if (!flw.ok) return new Response(JSON.stringify({ error: flw.errorMsg }), { status: 400 })
      const PRICE = 5000
      if (flw.amount < PRICE * 0.95) return new Response(JSON.stringify({ error: `Payment ₦${flw.amount} less than required ₦${PRICE}` }), { status: 400 })
      if (flw.currency !== 'NGN') return new Response(JSON.stringify({ error: `Invalid currency ${flw.currency}` }), { status: 400 })
      const txKey = `premium_${flw.flwId || txId}`
      const existing = await dbGet('payments', txKey)
      if (existing?.consumed) {
        if (existing.user_email !== userEmail) return new Response(JSON.stringify({ error: 'Transaction used by different account' }), { status: 409 })
        const u = await dbGet('users', userEmail); const rt = await createSession({ id: u?.id || session.sub, email: userEmail, premium: true })
        return new Response(JSON.stringify({ ok: true, message: 'Premium already active' }), { headers: { 'Set-Cookie': sessionCookie(rt), 'Content-Type': 'application/json' } })
      }
      const user = await dbGet('users', userEmail); if (!user) return new Response(JSON.stringify({ error: 'User not found' }), { status: 404 })
      await dbSet('payments', txKey, { flwId: flw.flwId, txId, txRef: flw.txRef, user_email: userEmail, amount: flw.amount, currency: flw.currency, product: 'premium', consumed: true, consumed_at: Date.now() })
      user.premium = true; user.premium_activated_at = Date.now(); user.premium_tx_id = txId; user.premium_provider = 'flutterwave'
      await dbSet('users', userEmail, user)
      const newToken = await createSession({ id: user.id, email: userEmail, premium: true })
      return new Response(JSON.stringify({ ok: true, message: 'CBT Premium activated!' }), { headers: { 'Set-Cookie': sessionCookie(newToken), 'Content-Type': 'application/json' } })
    } catch (e: any) { console.error('pay/verify:', e?.message); return new Response(JSON.stringify({ error: 'Server error: ' + (e?.message || '') }), { status: 500 }) }
  }

  if (path.endsWith('/pay/success')) {
    try {
      const cookie = req.headers.get('cookie') || ''; const token = getSessionToken(cookie)
      if (!token) return new Response(JSON.stringify({ error: 'Not authenticated' }), { status: 401 })
      let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ error: 'Invalid session' }), { status: 401 }) }
      const txId = qp(req.url, 'transaction_id') || qp(req.url, 'transactionId') || qp(req.url, 'tx_id')
      if (!txId) return new Response(JSON.stringify({ error: 'Missing transaction_id' }), { status: 400 })
      const userEmail = (session?.email || '').toLowerCase()
      const user = await dbGet('users', userEmail); if (!user) return new Response(JSON.stringify({ error: 'User not found' }), { status: 404 })
      const already = await dbGet('payments', `tx_${txId}`)
      if (already?.consumed) { const nt = await createSession({ id: user.id, email: user.email, premium: true }); return new Response(JSON.stringify({ ok: true }), { headers: { 'Set-Cookie': sessionCookie(nt), 'Content-Type': 'application/json' } }) }
      const flw = await verifyFlw(txId); if (!flw.ok) return new Response(JSON.stringify({ error: `Payment not verified: ${flw.errorMsg}` }), { status: 400 })
      if (flw.amount < 5000 * 0.95 || flw.currency !== 'NGN') return new Response(JSON.stringify({ error: 'Invalid payment' }), { status: 400 })
      await dbSet('payments', `tx_${txId}`, { id: txId, user_email: userEmail, amount: flw.amount, currency: flw.currency, consumed: true, consumed_at: Date.now() })
      user.premium = true; user.premium_activated_at = Date.now(); user.premium_tx_id = txId; user.premium_provider = 'flutterwave'
      await dbSet('users', userEmail, user)
      const newToken = await createSession({ id: user.id, email: user.email, premium: true })
      return new Response(JSON.stringify({ ok: true }), { headers: { 'Set-Cookie': sessionCookie(newToken), 'Content-Type': 'application/json' } })
    } catch (e: any) { console.error('pay/success:', e?.message); return new Response(JSON.stringify({ error: 'Server error' }), { status: 500 }) }
  }
