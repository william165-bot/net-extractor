...
export default async (req: Request) => {
  ...
  if (path.endsWith('/feedback/mine')) {
    ...
    const keys = await dbList('feedbacks'); const items = await Promise.all(keys.map(k => dbGet('feedbacks', k)))
    return new Response(JSON.stringify(items.filter((fb: any) => fb && fb.email === session.email).sort((a: any, b: any) => b.created_at - a.created_at)), { headers: { 'Content-Type': 'application/json' } })
  }

  if (req.method === 'GET') {
    const isAdmin = await requireAdmin(cookie); if (!isAdmin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 })
    const keys = await dbList('feedbacks'); const items = await Promise.all(keys.map(k => dbGet('feedbacks', k)))
    return new Response(JSON.stringify(items.filter(Boolean).sort((a: any, b: any) => b.created_at - a.created_at)), { headers: { 'Content-Type': 'application/json' } })
  }
  if (req.method === 'POST') {
    const token = getSessionToken(cookie); if (!token) return new Response(JSON.stringify({ error: 'Not authenticated' }), { status: 401 })
    let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ error: 'Invalid session' }), { status: 401 }) }
    const body: any = await req.json().catch(() => ({})); const { message, subject } = body
    if (!message?.trim()) return new Response(JSON.stringify({ error: 'Message required' }), { status: 400 })
    const id = `fb_${Date.now()}_${nanoid(6)}`
    await dbSet('feedbacks', id, { id, email: session.email, subject: subject || 'General Feedback', message, created_at: Date.now(), read: false })
    return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } })
  }
  if (req.method === 'PATCH') {
    const isAdmin = await requireAdmin(cookie); if (!isAdmin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 })
    const body: any = await req.json().catch(() => ({})); if (!body.id) return new Response(JSON.stringify({ error: 'id required' }), { status: 400 })
    const fb = await dbGet('feedbacks', body.id); if (!fb) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 })
    fb.read = true; if (body.admin_reply !== undefined) { fb.admin_reply = body.admin_reply; fb.replied_at = Date.now() }
    await dbSet('feedbacks', body.id, fb); return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } })
  }
