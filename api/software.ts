...
const SW_PRICE = 100000; const SW_MIN = SW_PRICE * 0.95

export default async (req: Request) => {
  const cookie = req.headers.get('cookie') || ''
  if (req.method === 'GET') {
    const token = getSessionToken(cookie); if (!token) return new Response(JSON.stringify({ unlocked: false, price: SW_PRICE }), { headers: { 'Content-Type': 'application/json' } })
    let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ unlocked: false, price: SW_PRICE }), { headers: { 'Content-Type': 'application/json' } }) }
    if (session.role === 'admin') { const dlUrl = await dbGet('settings', 'software_download_url') || ''; return new Response(JSON.stringify({ unlocked: true, download_url: dlUrl, price: SW_PRICE }), { headers: { 'Content-Type': 'application/json' } }) }
    const user = await dbGet('users', session.email.toLowerCase())
    if (user?.software_unlocked === true) { const dlUrl = await dbGet('settings', 'software_download_url') || ''; return new Response(JSON.stringify({ unlocked: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
    return new Response(JSON.stringify({ unlocked: false, price: SW_PRICE }), { headers: { 'Content-Type': 'application/json' } })
  }
  if (req.method === 'POST') {
    const body: any = await req.json().catch(() => ({}))
    if (body.action === 'update') { const isAdmin = await requireAdmin(cookie); if (!isAdmin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 }); if (body.download_url !== undefined) await dbSet('settings', 'software_download_url', body.download_url); if (body.price !== undefined) await dbSet('settings', 'software_price', String(body.price)); return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } }) }
    if (body.action === 'unlock') {
      const token = getSessionToken(cookie); if (!token) return new Response(JSON.stringify({ error: 'Not authenticated' }), { status: 401 })
      ...
      const txId = String(body.transaction_id || '').trim(); if (!txId) return new Response(JSON.stringify({ error: 'Transaction ID required' }), { status: 400 }); if (!/^\d+$/.test(txId)) return new Response(JSON.stringify({ error: 'Invalid Transaction ID' }), { status: 400 })
      const userEmail = session.email.toLowerCase(); const user = await dbGet('users', userEmail); if (!user) return new Response(JSON.stringify({ error: 'User not found' }), { status: 404 })
      if (user.software_unlocked === true) { const dlUrl = await dbGet('settings', 'software_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
      const flw = await verifyFlw(txId); if (!flw.ok) return new Response(JSON.stringify({ error: flw.errorMsg }), { status: 400 })
      if (flw.amount < SW_MIN) return new Response(JSON.stringify({ error: `Payment ₦${flw.amount} less than ₦${SW_PRICE}` }), { status: 400 }); if (flw.currency !== 'NGN') return new Response(JSON.stringify({ error: 'Only NGN accepted' }), { status: 400 })
      const txKey = `sw_${flw.flwId || txId}`; const existing = await dbGet('payments', txKey)
      if (existing?.consumed) { if (existing.user_email !== userEmail) return new Response(JSON.stringify({ error: 'Transaction used by another account' }), { status: 409 }); user.software_unlocked = true; await dbSet('users', userEmail, user); const dlUrl = await dbGet('settings', 'software_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
      await dbSet('payments', txKey, { flwId: flw.flwId, txId, txRef: flw.txRef, user_email: userEmail, amount: flw.amount, currency: flw.currency, product: 'software', consumed: true, consumed_at: Date.now() })
      user.software_unlocked = true; user.software_unlocked_at = Date.now(); user.software_tx_id = txId; await dbSet('users', userEmail, user)
      const dlUrl = await dbGet('settings', 'software_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } })
    }
