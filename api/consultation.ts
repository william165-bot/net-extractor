...
export default async (req: Request) => {
  ...
  if (req.method === 'GET') {
    ...
    const keys = await dbList('consultations'); const items = await Promise.all(keys.map(k => dbGet('consultations', k)))
    const all = items.filter(Boolean).sort((a: any, b: any) => b.created_at - a.created_at)
    if (session.role === 'admin') return new Response(JSON.stringify(all), { headers: { 'Content-Type': 'application/json' } })
    return new Response(JSON.stringify(all.filter((c: any) => c.email === session.email)), { headers: { 'Content-Type': 'application/json' } })
  }
  if (req.method === 'POST') {
    const body: any = await req.json().catch(() => ({})); const { action } = body
    if (action === 'submit') {
      const token = getSessionToken(cookie); if (!token) return new Response(JSON.stringify({ error: 'Not authenticated' }), { status: 401 })
      let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ error: 'Invalid session' }), { status: 401 }) }
      if (session.role === 'admin') return new Response(JSON.stringify({ error: 'Admin cannot submit' }), { status: 403 })
      const { topic, details } = body; if (!topic?.trim() || !details?.trim()) return new Response(JSON.stringify({ error: 'Topic and details required' }), { status: 400 })
      const id = `req_${Date.now()}_${nanoid(6)}`
      await dbSet('consultations', id, { id, email: session.email, topic, details, status: 'pending', created_at: Date.now(), admin_reply: null, payment_link: null, session_time: null, updated_at: null })
      return new Response(JSON.stringify({ ok: true, id }), { headers: { 'Content-Type': 'application/json' } })
    }
    if (action === 'reply' || action === 'close') {
      const isAdmin = await requireAdmin(cookie); if (!isAdmin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 })
      const { id } = body; if (!id) return new Response(JSON.stringify({ error: 'id required' }), { status: 400 })
      const record = await dbGet('consultations', id); if (!record) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 })
      if (action === 'reply') { const { reply, payment_link, session_time } = body; if (reply) record.admin_reply = reply; if (payment_link) record.payment_link = payment_link; if (session_time) record.session_time = session_time; record.status = payment_link ? 'payment_sent' : 'replied' }
      else record.status = 'closed'
      record.updated_at = Date.now(); await dbSet('consultations', id, record)
