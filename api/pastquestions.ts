...
const PQ_PRICE = 7000; const PQ_MIN = PQ_PRICE * 0.95

export default async (req: Request) => {
  const cookie = req.headers.get('cookie') || ''
  if (req.method === 'GET') {
    const token = getSessionToken(cookie); if (!token) return new Response(JSON.stringify({ unlocked: false, price: PQ_PRICE }), { headers: { 'Content-Type': 'application/json' } })
    let session: any; try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ unlocked: false, price: PQ_PRICE }), { headers: { 'Content-Type': 'application/json' } }) }
    if (session.role === 'admin') { const dlUrl = await dbGet('settings', 'pastq_download_url') || ''; return new Response(JSON.stringify({ unlocked: true, download_url: dlUrl, price: PQ_PRICE }), { headers: { 'Content-Type': 'application/json' } }) }
    const user = await dbGet('users', session.email.toLowerCase())
    if (user?.pastq_unlocked === true) { const dlUrl = await dbGet('settings', 'pastq_download_url') || ''; return new Response(JSON.stringify({ unlocked: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
    return new Response(JSON.stringify({ unlocked: false, price: PQ_PRICE }), { headers: { 'Content-Type': 'application/json' } })
  }
  if (req.method === 'POST') {
    const body: any = await req.json().catch(() => ({}))
    if (body.action === 'update') { const isAdmin = await requireAdmin(cookie); if (!isAdmin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 }); if (body.download_url !== undefined) await dbSet('settings', 'pastq_download_url', body.download_url); return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } }) }
    if (body.action === 'unlock') {
      ...
      if (user.pastq_unlocked === true) { const dlUrl = await dbGet('settings', 'pastq_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
      const flw = await verifyFlw(txId); if (!flw.ok) return new Response(JSON.stringify({ error: flw.errorMsg }), { status: 400 })
      if (flw.amount < PQ_MIN) return new Response(JSON.stringify({ error: `Payment ₦${flw.amount} less than ₦${PQ_PRICE}` }), { status: 400 }); if (flw.currency !== 'NGN') return new Response(JSON.stringify({ error: 'Only NGN accepted' }), { status: 400 })
      const txKey = `pq_${flw.flwId || txId}`; const existing = await dbGet('payments', txKey)
      if (existing?.consumed) { if (existing.user_email !== userEmail) return new Response(JSON.stringify({ error: 'Transaction used by another account' }), { status: 409 }); user.pastq_unlocked = true; await dbSet('users', userEmail, user); const dlUrl = await dbGet('settings', 'pastq_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } }) }
      await dbSet('payments', txKey, { flwId: flw.flwId, txId, txRef: flw.txRef, user_email: userEmail, amount: flw.amount, currency: flw.currency, product: 'pastquestions', consumed: true, consumed_at: Date.now() })
      user.pastq_unlocked = true; user.pastq_unlocked_at = Date.now(); user.pastq_tx_id = txId; await dbSet('users', userEmail, user)
      const dlUrl = await dbGet('settings', 'pastq_download_url') || ''; return new Response(JSON.stringify({ ok: true, download_url: dlUrl }), { headers: { 'Content-Type': 'application/json' } })
    }
