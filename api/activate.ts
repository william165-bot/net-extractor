...
export default async (req: Request) => {
  ...
  try {
    ...
    let session: any
    try { session = await verifySession(token) } catch { return new Response(JSON.stringify({ error: 'Invalid session' }), { status: 401, headers: { 'Content-Type': 'application/json' } }) }
    const body: any = await req.json().catch(() => ({}))
    const { activationKey } = body
    if (!activationKey) return new Response(JSON.stringify({ error: 'activationKey required' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    const userEmail = session.email.toLowerCase()
    const keyRecord = await dbGet('keys', activationKey)
    if (!keyRecord) return new Response(JSON.stringify({ error: 'Invalid activation key' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    if (keyRecord.used) return new Response(JSON.stringify({ error: 'Key already used' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    if (keyRecord.assigned_email && keyRecord.assigned_email.toLowerCase() !== userEmail) return new Response(JSON.stringify({ error: 'Key assigned to different account' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    keyRecord.used = true; keyRecord.used_by = userEmail; keyRecord.used_at = Date.now()
    await dbSet('keys', activationKey, keyRecord)
    ...
    user.premium = true; user.premium_activated_at = Date.now(); user.premium_key = activationKey
    ...
    return new Response(JSON.stringify({ error: 'Activation failed: ' + (e?.message || 'server error') }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
}
